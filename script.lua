-- FireHub For Delta Executor (No Drawing, No Synapse functions)
...
-- Safe request function for Delta
...
-- REMOVED ALL DRAWING FUNCTIONS (FOV Circle removed)
            ...
            if player ~= LocalPlayer then
                ...
                if success and char then
                    ...
                    if hrp and humanoid and humanoid.Health > 0 then
                        local distance = (hrp.Position - (LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") and LocalPlayer.Character.HumanoidRootPart.Position or Vector3.new())).Magnitude
...
local function GetNearestBoss()
    ...
    local bosses = {
        ...
        "Fajita", "Order", "Lady", "Beautiful Pirate", "Bob"
    }
...
local function GetNearestMaterial()
    ...
    local materials = {"Scrap", "Metal", "Wood", "Bone", "Fragment", "Magma Ore"}
    local closest = nil
    local dist = math.huge
...
RunService.Heartbeat:Connect(function()
    ...
    if Features.AutoMaterial then
        ...
        if mat then
            ...
            if dist < 10 then
                local prompt = mat:FindFirstChildOfClass("ProximityPrompt")
                if prompt then
                    fireproximityprompt(prompt)
                end
            else
...
local function SwitchTab(tabName)
    ...
    if tabName == "HOME" then
        ...
        CreateButton(ScrollingFrame, UDim2.new(0, 10, 0, yPos), "COPY LINK", function()
            setclipboard("FireHub For Delta")
        end)
        yPos = yPos + (isMobile and 65 or 60)
    ...
    elseif tabName == "PVP" then
        ...
        -- FOV Circle toggle removed for Delta compatibility

        CreateSlider(ScrollingFrame, UDim2.new(0, 10, 0, yPos), "Aim Distance", 50, 300, Features.AimFOV, function(value)
